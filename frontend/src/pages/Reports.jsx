...
const mergeDateRanges = (ranges) => {
  ...
  const merged = [{ start: new Date(validRanges[0].start), end: new Date(validRanges[0].end) }]
...
const calculateDowntimeFromErrors = (errors, periodStart = null, periodEnd = null) => {
  ...
  errors.forEach((error) => {
    ...
    const rawStart = error.error_date || error.created_at
    const rawEnd = error.resolved_at || error.updated_at
    ...
    if (periodStart) {
      const reportStart = new Date(periodStart)
      if (start < reportStart) start = new Date(reportStart)
    }
    if (periodEnd) {
      const reportEnd = new Date(periodEnd)
      if (end > reportEnd) end = new Date(reportEnd)
    }
    if (end < start) return
  ...
  return mergedRanges.reduce(
    (total, range) => total + calculateWorkingDays(range.start, range.end),
    0
  )
}
...
const calculateAvailability = (downtimeDays) => {
  const downtime = Math.max(0, Number(downtimeDays) || 0)
  const availableDays = Math.max(0, TOTAL_WORKING_DAYS - downtime)
  const availability = (availableDays / TOTAL_WORKING_DAYS) * 100
...
const Reports = () => {
  ...
  const generateReport = useCallback(async () => {
    ...
    try {
      ...
      let periodStart = filters.date_from ? new Date(filters.date_from) : new Date('2000-01-01T00:00:00')
      let periodEnd = filters.date_to ? new Date(filters.date_to) : new Date()

      if (Number.isNaN(periodStart.getTime())) {
        periodStart = new Date('2000-01-01T00:00:00')
      }
      ...
      const isErrorInPeriod = (error) => {
        const startDate = error.error_date || error.created_at
        const resolvedDate = error.resolved_at || error.updated_at
        if (!startDate || !resolvedDate) return false
        const errorStart = new Date(startDate)
        const errorEnd = new Date(resolvedDate)
        if (!isValidDate(errorStart) || !isValidDate(errorEnd)) return false
        return errorStart <= periodEnd && errorEnd >= periodStart
      }
      ...
      let items = equipment.map((eq) => {
        const eqErrors = errors.filter(
          (e) => Number(e.equipment_id) === Number(eq.id) && isErrorInPeriod(e)
        )
        ...
        const downtimeWorkingDays = calculateDowntimeFromErrors(
          resolvedErrors,
          periodStart,
          periodEnd
        )
        const availability = calculateAvailability(downtimeWorkingDays)
        ...
        return {
          ...
          open_errors: eqErrors.length - resolvedErrors.length,
          total_repairs: eqRepairs.length,
          completed_repairs: eqRepairs.filter(
            (r) => String(r.status || '').toLowerCase() === 'completed'
          ).length,
          total_downtime_days: downtimeWorkingDays,
      ...
      if (filters.hospital_id) {
        items = items.filter((item) => Number(item.hospital_id) === Number(filters.hospital_id))
      }
      if (filters.equipment_id) {
        items = items.filter((item) => Number(item.id) === Number(filters.equipment_id))
      }
      if (filters.status) {
        items = items.filter(
          (item) =>
            String(item.current_status || '').toLowerCase() ===
            String(filters.status || '').toLowerCase()
        )
      }
      if (filters.date_from) {
        const fromDate = startOfDay(new Date(filters.date_from))
        items = items.filter((item) => {
          if (!item.equipment_added_on) return false
          const added = new Date(item.equipment_added_on)
          return isValidDate(added) && added >= fromDate
        })
      }
      if (filters.date_to) {
        const toDate = endOfDay(new Date(filters.date_to))
        items = items.filter((item) => {
          if (!item.equipment_added_on) return false
          const added = new Date(item.equipment_added_on)
          return isValidDate(added) && added <= toDate
        })
      }
      if (filters.min_availability !== '') {
        const minAvail = Number(filters.min_availability)
        items = items.filter((item) => (item.availability_percentage ?? 0) >= minAvail)
      }
      if (filters.max_downtime !== '') {
        const maxDowntime = Number(filters.max_downtime)
        items = items.filter((item) => (item.total_downtime_days ?? 0) <= maxDowntime)
      }

      if (filters.functional_status) {
        const isFunctionalFilter = filters.functional_status === 'Functional'
        items = items.filter((item) => {
          const avail = item.availability_percentage ?? 0
          return (avail >= 90) === isFunctionalFilter
        })
      }
      ...
      const functionalCount = items.filter((i) => (i.availability_percentage ?? 0) >= 90).length
      const nonFunctionalCount = totalEquipment - functionalCount
      const totalErrors = items.reduce((sum, i) => sum + (Number(i.total_errors) || 0), 0)
      const totalRepairs = items.reduce((sum, i) => sum + (Number(i.total_repairs) || 0), 0)
      const totalDowntimeDays = items.reduce(
        (sum, i) => sum + (Number(i.total_downtime_days) || 0),
        0
      )
      const avgAvailability =
        items.length > 0
          ? items.reduce((sum, i) => sum + (i.availability_percentage ?? 0), 0) / items.length
          : 0
  ...
  const getActiveFilterCount = () => {
    ...
    if (filters.functional_status) count++
    return count
  }
  ...
  const handleOpenEditStatus = (item) => {
  ...
  const handleSaveStatus = async () => {
    ...
    try {
      await api.patch(`/equipment/${editingEquipmentId}`, { status: editStatus })
  ...
  const exportToCSV = () => {
    ...
    try {
      ...
      displayData.forEach((item) => {
        const avail = item.availability_percentage ?? 0
        const functionalStatus = avail >= 90 ? 'Functional' : 'Non-Functional'
        const row = [
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      XLSX.writeFile(
        wb,
        `equipment_report_${new Date().toISOString().slice(0, 10)}.xlsx`
      )
  ...
  const exportToPDF = () => {
    ...
    try {
      ...
      if (!printWindow) {
        toast.warning('Please allow pop-ups')
        return
      }
      const headers = [
        'Equipment',
      ...
      const totalDowntime = displayData.reduce(
        (sum, item) => sum + (Number(item.total_downtime_days) || 0),
        0
      )
      const avgAvailability =
        displayData.length > 0
          ? displayData.reduce((sum, item) => sum + (item.availability_percentage ?? 0), 0) /
            displayData.length
          : 0

      const rows = displayData
        .map((item) => {
          const downtime = Number(item.total_downtime_days) || 0
          const availability = item.availability_percentage ?? 0
          const isFunctional = availability >= 90
          const functionalLabel = isFunctional ? 'Functional' : 'Non-Functional'
          const functionalColor = isFunctional ? '#22C55E' : '#EF4444'
          const downtimeColor = downtime > 10 ? '#EF4444' : downtime > 5 ? '#F59E0B' : '#0F172A'
          return `
                <tr>
                  ...
                  <td style="font-weight:700;color:${downtimeColor}">
                    ${safeToFixed(downtime, 2)}d
                  </td>
                  <td style="font-weight:700;color:${functionalColor}">
                    ${functionalLabel}
                  </td>
                </tr>
              `
        ...
        <html>
          ...
          <body>
            ...
            <div class="summary">
              ...
              <div class="card"><div class="label">Total Downtime</div><div class="value">${safeToFixed(totalDowntime, 2)}d</div></div>
    ...
    >
      ...
      >
        <Box>
          ...
          <Typography variant="body2" sx={{ color: colors.lightText, mt: 1 }}>
            Equipment analysis with downtime and functional status based on {TOTAL_WORKING_DAYS} working
            days.
          </Typography>
        </Box>
      ...
      >
        <MenuItem onClick={exportToCSV}>
          <FileDownload sx={{ mr: 1.5, color: '#3B82F6' }} />
          CSV
        </MenuItem>
        <MenuItem onClick={exportToExcel}>
          <TableChart sx={{ mr: 1.5, color: '#22C55E' }} />
          Excel
        </MenuItem>
        <MenuItem onClick={exportToPDF}>
          <PictureAsPdf sx={{ mr: 1.5, color: '#EF4444' }} />
          PDF
        </MenuItem>
      </Menu>
          ...
          <Table size="small" sx={{ minWidth: 1100 }}>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={10} align="center" sx={{ py: 5 }}>
                    <Typography sx={{ color: colors.lightText }}>No data found</Typography>
                ...
                displayData.map((item, index) => {
                  const availability = item.availability_percentage ?? 0
                  const downtimeDays = Number(item.total_downtime_days) || 0

                  const downtimeColor =
                    downtimeDays > 10 ? colors.error : downtimeDays > 5 ? colors.warning : colors.darkNavy

                  const isFunctional = availability >= 90
                  const functionalLabel = isFunctional ? 'Functional' : 'Non-Functional'
                  const functionalColor = isFunctional ? colors.success : colors.error
                  ...
                  return (
                    <TableRow key={item.id ?? index} hover>
                      ...
                      <TableCell sx={{ color: colors.lightText }}>{item.hospital_name}</TableCell>
                      <TableCell sx={{ color: colors.lightText }}>{item.department_name}</TableCell>
      ...
      <Dialog open={openViewDialog} onClose={() => setOpenViewDialog(false)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ bgcolor: colors.darkNavy, color: 'white' }}>
        ...
        <DialogContent dividers sx={{ py: 3 }}>
          {selectedItem && (
            <>
              <Paper sx={{ p: 2.5, mb: 2, bgcolor: 'rgba(103,232,249,0.04)', borderRadius: 3 }}>
                <Typography variant="h5" fontWeight={700} sx={{ color: colors.darkNavy }}>
                  {selectedItem.equipment_name}
                </Typography>
                <Box sx={{ display: 'flex', gap: 1, mt: 1, flexWrap: 'wrap' }}>
                  <Chip label={`Model: ${selectedItem.model}`} size="small" />
                  <Chip label={`SN: ${selectedItem.serial_number}`} size="small" />
                  <Chip
                  ...
                  />
                  {(() => {
                    const avail = selectedItem.availability_percentage ?? 0
                    const isFunc = avail >= 90
                    return (
                      <Chip
                        label={isFunc ? 'Functional' : 'Non-Functional'}
                        size="small"
                        sx={{
                          bgcolor: isFunc ? colors.success : colors.error,
                          color: 'white',
                          fontWeight: 700,
                        }}
                      />
                ...
                {(user?.role === 'ENGINEER' || user?.role === 'SUPER_USER') && (
                  <Button
                    variant="outlined"
                    size="small"
                    startIcon={<Edit />}
                    onClick={() => handleOpenEditStatus(selectedItem)}
                    sx={{ mt: 2 }}
                  >
                    Edit Status
                  </Button>
              ...
              >
                <Paper sx={{ p: 2, borderRadius: 2 }}>
                  ...
                  <MetricRow label="Hospital" value={selectedItem.hospital_name} />
                  <MetricRow label="Department" value={selectedItem.department_name} />
                  <MetricRow label="Location" value={selectedItem.location} />
                  <MetricRow label="Added On" value={formatDate(selectedItem.equipment_added_on)} />
                </Paper>
                ...
                <Paper sx={{ p: 2, borderRadius: 2 }}>
                  ...
                  <MetricRow label="Total Errors" value={selectedItem.total_errors ?? 0} />
                  <MetricRow label="Resolved Errors" value={selectedItem.resolved_errors ?? 0} />
                  <MetricRow label="Open Errors" value={selectedItem.open_errors ?? 0} />
                  <Divider sx={{ my: 1 }} />
                  <MetricRow label="Total Repairs" value={selectedItem.total_repairs ?? 0} />
                  <MetricRow
                    label="Completed Repairs"
                    value={selectedItem.completed_repairs ?? 0}
                  />
                  <Divider sx={{ my: 1 }} />
                  <MetricRow
                    label="Unique Downtime (Working Days)"
                    value={`${safeToFixed(selectedItem.total_downtime_days ?? 0, 2)}d`}
                  />
                  <MetricRow
                    label="Functional Status"
                    value={(selectedItem.availability_percentage ?? 0) >= 90 ? 'Functional' : 'Non-Functional'}
                    bold
                  />
                </Paper>
              </Box>
      ...
      <Dialog open={openEditDialog} onClose={() => setOpenEditDialog(false)} maxWidth="xs" fullWidth>
        <DialogTitle>Edit Equipment Status</DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          ...
          <Button onClick={handleSaveStatus} variant="contained" sx={{ bgcolor: colors.darkNavy }}>
            Save
          </Button>
        </DialogActions>
      </Dialog>
...
const MetricRow = ({ label, value, bold = false }) => {
  return (
    <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 2, py: 0.6 }}>
      ...
      <Typography
        ...
        sx={{ color: bold ? colors.darkNavy : 'inherit' }}
      >
        {value}
      </Typography>
    </Box>
